...
@section('title', 'Laravel チュートリアル初級')

@section('content')
<h3>従業員登録画面</h3>
<p><span class="label label-danger">入力画面</span> -> 確認画面 -> 完了画面</p>   

<form action="{{ route('insert.confirm') }}" method="POST" class="form-horizonal">
    <input type="hidden" name="_token" value="{{ csrf_token() }}">
    <div class="form-group @if($errors->has('username')) has-error @endif">
    <label for="username" class="col-sm-2 control-label">名前：</label>
    <div class="col-sm-10">
    <input type="text" class="form-control" id="username" name="username" placeholder="名前を入力してください。" value="{{ old('username') }}">
